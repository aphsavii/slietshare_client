...
import { formatTimestamp, textCapitalize, toMonthYear, trimText } from "@/helpers";
...
function Me() {
  ...
  useEffect(() => {
    ...
    userService
      .getProfileViews(userData.regno)
      .then((data) => {
            ...
            <div className=" mx-auto  w-full flex flex-col lg:flex-row py-10 md:py-16 px-3 lg:px-0">
              <div className="lg:max-w-[400px] min-w-64 lg:min-w-80 flex flex-col gap-5 ">
                ...
                {userData.role == "admin" && (
                  <div className="py-5 px-6 bg-white h-fit rounded-xl border border-1 border-lightGray w-full text-lg lg:text-xl font-semibold text-lightBlack">
                    <h2>
                      <Eye size={20} className="inline mr-2 font-semibold" />
                      <span className="">
                        Profile Views{" "}
                      ...
                      <div className="w-full flex flex-col">
                        {profileViews?.map((view, index) => (
                          <div key={index} className="py-3 ">
                            ...
                            <span className="ml-2 mt-2 text-gray-400 text-[10px] lg:text-xs font-normal float-right absolute top-0 right-0">{formatTimestamp(view.viewTime)}</span>
                              <Avatar className="h-10 w-10 bg-center bg-cover rounded-sm mr-4 mt-1">
                              ...
                              <div >
                                <Link to={`/user/${view.regno}`} className>
                                <h2 className="text-base lg:text-lg font-medium hover:underline flex justify-between w-full">
                                  {view.fullName} 
                                </h2>
                               
                                </Link>
                                <div className="text-[10px] md:text-xs font-light leading-3">
                                  <span>{trimText(view.headLine,35)} </span>
                                </div>
                              </div>
                            </div>
                          </div>
